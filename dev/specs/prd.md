...
**Backboard** is a lightweight, local-first task management tool focused on **what's current** – not
historical data. Users can start immediately without an account, with all data stored locally.
Account creation enables cross-device sync.
...
When a Scope has tasks in "Now" but is **not scheduled for today** (no ScheduleSlot for today's
date), the UI displays a warning state:
...
This is a **destructive hint** pattern – guiding users that these tasks shouldn't be in "Now"
because the scope isn't in focus today. The goal is to help users maintain a clean, focused task
list aligned with their schedule.

**Trigger condition**: Scope has `status: "now"` tasks AND no `ScheduleSlot` exists where
`date === today` AND `scopeId === scope.id`.
...
- **Rows**: ALL non-archived Scopes + Projects active in current month (if week spans months, show
  projects active in EITHER month)
...
Since Backboard is **local-first**, traditional server-side cron jobs don't apply. Instead, all
background tasks run **client-side on app launch** (and optionally via a manual "Sync" button).
...
- Optional: Add a manual "Sync" button in the UI for users who leave the app open for extended
  periods
...
The Figma designs provided are **desktop layouts only**. However, mobile support is required for
MVP.
